export const FAQ_ITEMS = [
    {
      question: "How secure is Encrypt Bank?",
      answer: "We use end-to-end encryption for all transactions and implement zero-knowledge protocols to ensure your financial data remains private and secure."
    },
    {
      question: "How do I reset my password?",
      answer: "You can reset your password by clicking 'Forgot Password' on the login page. We'll send you a secure reset link to your registered email."
    },
    {
      question: "What are the daily transaction limits?",
      answer: "Standard accounts have a daily limit of $10,000 for transfers and $5,000 for withdrawals. These limits can be adjusted based on your account status."
    }
  ];
  
  export const CONTACT_METHODS = [
    {
      icon: "fa-phone",
      title: "Phone Support",
      detail: "1-800-ENCRYPT",
      subDetail: "Available 24/7"
    },
    {
      icon: "fa-envelope",
      title: "Email",
      detail: "support@encryptbank.com",
      subDetail: "Response within 24 hours"
    },
    {
      icon: "fa-message",
      title: "Live Chat",
      detail: "Chat with us",
      subDetail: "Available 9AM-6PM CST"
    }
  ];